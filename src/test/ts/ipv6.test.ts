...
import { isV6Format, Address } from '../../main/ts/core.ts'

describe('ipv6', () => {
  beforeAll(() => Address.strict = false)
  afterAll(() => Address.strict = true)

  function ipv6test(is: 1 | false, addr: string) {
    const expected = !!is
    test(`${addr} should ${expected ? '' : 'not '}be valid`, () => {
      expect(isV6Format(addr)).toBe(expected)
    })
  }
